...
- Inserts ONLY the text_plain field into GoodBook.db
- Matches by Book, Chapter, and Verse

Usage:
    python import_text_to_goodbook.py
"""
...
def create_schema(conn: sqlite3.Connection):
    """Create books, chapters, and verses tables if they don't exist."""
    conn.executescript("""
...
def insert_or_update_verse(
    ...
    if row:
        # Update existing
        cur.execute(
            "UPDATE verses SET text_plain = ? WHERE id = ?",
            (text_plain, row[0])
        )
    else:
        # Insert new
        cur.execute(
            "INSERT INTO verses (book_id, chapter_id, verse_number, text_plain) VALUES (?, ?, ?, ?)",
            (book_id, chapter_id, verse_num, text_plain)
        )
...
def main():
    ...
    print(f"Importing text_plain from {CACHE_ROOT} into {DB_PATH}")
    print("=" * 60)
        ...
        for chapter_file in chapter_files:
            ...
            for verse_obj in verses:
                ...
                insert_or_update_verse(
                    conn, book_id, chapter_id, int(verse_num), text_plain
                )
                total_verses += 1
