...
# This function scrapes a single chapter of a given book and returns the passage text and footnotes
        ...
        if footnotes_section and isinstance(footnotes_section, Tag):
            ...
            for footnote in footnote_elements:
                if isinstance(footnote, Tag):
                    ...
                    if verse_ref and isinstance(verse_ref, Tag):
    ...
    def get_valid_book_name(self, book_name):
        """Validate and normalize book name, handling abbreviations."""
        # First check if it's already a valid full book name
        if book_name in self.BOOK_CHAPTERS:
            logging.info(f"Valid book name: {book_name}")
            return book_name
        
        # Try to resolve from abbreviations
        try:
            resolved_name = self.resolve_abbreviation(book_name)
            logging.info(f"Resolved abbreviation '{book_name}' to '{resolved_name}'")
            return resolved_name
        except ValueError:
            logging.warning(f"Invalid book name or abbreviation: {book_name}")
            return None
    ...
    def extract_chapter_subtitles_from_html(self, book_name, chapter_number):
        ...
        unclassed_h3s = [h3.get_text(strip=True) for h3 in soup.find_all("h3") if isinstance(h3, Tag) and not h3.has_attr("class")]
