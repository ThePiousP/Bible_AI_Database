...
# =========================
# WHITELISTS (nested, with canonical case)
# =========================
WHITELIST_TAGS = ["BIBLE_FLORA", "BIBLE_NATIONS", "DEITY", "TIME"]

# lookup:  { TAG: set(lowercase_name) }
WHITELIST_LOOKUP = {tag: set() for tag in WHITELIST_TAGS}
# canonical: { TAG: { lowercase_name: OriginalCasedName } }
WHITELIST_CANONICAL = {tag: {} for tag in WHITELIST_TAGS}

def load_nested_names(tag_name: str):
    """Load {tag: {Name: {...}}} and preserve original capitalization in canonical map."""
    ...
    try:
        ...
        if isinstance(group, dict):
            for name in group.keys():
                low = str(name).lower()
                WHITELIST_LOOKUP[tag_name].add(low)
                WHITELIST_CANONICAL[tag_name].setdefault(low, name)
    except Exception as e:
...
# =========================
# Strict DEITY allow‑list (extended from DEITY.json)
# =========================
BASE_DEITY_ALLOW = {
    "yhwh", "yahweh", "jehovah", "god the father",
    "jesus", "jesus christ", "christ jesus", "the christ",
    "holy spirit", "spirit of god", "spirit of the lord",
}
DEITY_ALLOW = set(BASE_DEITY_ALLOW)
for low_name in WHITELIST_LOOKUP.get("DEITY", set()):
    DEITY_ALLOW.add(low_name)

# =========================
# Tagging rules (preserve capitalization; pronouns removed for PERSON)
# =========================
TAG_RULES = {
    "PERSON": {
...
# =========================
# Helpers (KJV parsing) — note: NEVER lowercase tokens
# =========================
...
for row in rows:
    ...
    # 1) Try to hit a whitelist name (case-insensitive -> recover canonical casing)
    for w in words:
        cand = w.strip()
        if not cand:
            continue
        ...
        low = cand.lower()
        hit_tag = None
        for tag, lookup in WHITELIST_LOOKUP.items():
            if low in lookup:
                hit_tag = tag
                canonical_name = WHITELIST_CANONICAL[tag][low]  # exact casing from file
                break
        if canonical_name:
            break
    ...
    # Strict DEITY allow‑list: demote non-allowed DEITY to ambiguous
    if "DEITY" in accepted:
        name_low = canonical_name.lower()
        if name_low not in DEITY_ALLOW:
